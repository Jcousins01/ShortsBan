...
// Edge/Bing uses chrome.* API (same as Chrome)
...
async function showBadgeOrNotify(blockedUrl, restoreUrl) {
  // Edge doesn't support auto-opening popups, so we use badge and notification
  try {
    await chrome.action.setBadgeText({ text: "!" });
    await chrome.action.setBadgeBackgroundColor({ color: "#d70022" });
  } catch (error) {
    console.warn("ShortsBan could not set badge:", error);
  }
